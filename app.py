...
# Initialize or repair the database file
if not os.path.exists(db_file):
    ...
    for col in base_headers:
        if col not in _df.columns:
            ...
            needs_save = True
    if needs_save:
        _df.to_csv(db_file, index=False)
        ...
        if lat is not None and lon is not None:
            ...
            if year is not None:
                ...
                if year_data and norm_data:
                    ...
                    # Custom Calculation: 3-Month Prior Temperature Mean & Anomaly
                    if doy is not None:
                        ty, tm = calc_prior_3_months(year, doy)
                        if ty and tm:
                            prev_year_data = {}
                            if min(ty) < year:
                                prev_year_data = get_climate_data(lat, lon, el, f"Year_{min(ty)}")
...
with st.sidebar.expander("🌐 Fetch from GBIF", expanded=False):
    ...
    g_counties = st.text_input("County(s):", help="Comma-separated (e.g. Weber, Davis)", key="g_counties")
    ...
    if st.button("Fetch & Process GBIF", type="primary", use_container_width=True):
        with st.spinner("Paginating GBIF..."):
            ...
            raw_records, seen_col_nums, offset, end_of_records = [], set(), 0, False
...
with tab1:
    ...
    # Auto-flag manually checked records as scored
...
with tab2:
    st.subheader("Interactive Geographic Distribution")
    map_df = edited_df.dropna(subset=['Latitude', 'Longitude']).copy()
    if map_df.empty:
        st.warning("No coordinate data available to map.")
    else:
        map_df['Data_Source'] = map_df['Data_Source'].fillna('Unknown')
        fig_map = px.scatter_mapbox(
            map_df, lat="Latitude", lon="Longitude", color="Species",
...
with tab3:
    if edited_df.empty: st.info("Ledger is empty. Add data to view graphs.")
    else:
        c1, c2 = st.columns(2)
        sources = edited_df['Data_Source'].fillna('Unknown').unique().tolist()
        species = edited_df['Species'].dropna().unique().tolist()
        with c1: sel_src = st.multiselect("Data Source:", sources, default=sources)
        with c2: sel_spp = st.multiselect("Species:", species, default=species)
        
        plot_df = edited_df[edited_df['Data_Source'].isin(sel_src) & edited_df['Species'].isin(sel_spp)].copy()
        ...
        if len(num_cols) >= 2:
            ...
            with s_c1: selected_x = st.selectbox("X-Axis:", num_cols, index=num_cols.index('Year') if 'Year' in num_cols else 0)
            with s_c2: selected_y = st.selectbox("Y-Axis:", num_cols, index=num_cols.index('DOY') if 'DOY' in num_cols else 1)
            
            valid_plot_df = plot_df.dropna(subset=[selected_x, selected_y])
            
            st.markdown("---")
            use_outlier_filter = st.checkbox(f"Enable Outlier Filtering for {selected_y}", value=False)
            if use_outlier_filter:
                std_devs = st.slider("Keep data within standard deviations:", 1.0, 5.0, 2.5, 0.1)
                mean_y = valid_plot_df[selected_y].mean()
                std_y = valid_plot_df[selected_y].std()
                valid_plot_df = valid_plot_df[np.abs(valid_plot_df[selected_y] - mean_y) <= (std_devs * std_y)]
                st.info(f"Filtered out points outside {mean_y:.1f} ± {std_devs*std_y:.1f}")

            if len(valid_plot_df) >= 2:
                fig = px.scatter(
                    valid_plot_df, x=selected_x, y=selected_y, color='Species' if 'Species' in valid_plot_df.columns else None,
                    symbol='Data_Source' if 'Data_Source' in valid_plot_df.columns else None,
                    hover_data=['Collector', 'Year', 'DOY'], template="streamlit"
                )
                
                x_v, y_v = valid_plot_df[selected_x].values, valid_plot_df[selected_y].values
                if np.var(x_v) > 0 and np.var(y_v) > 0:
                    ...
                    fig.add_scatter(x=lx, y=slope * lx + intercept, mode='lines', name='Trend', line=dict(color='black', dash='dash'))
                    st.success(f"📈 **Trendline:** y = {slope:.3f}x {'+' if intercept>=0 else '-'} {abs(intercept):.3f}  |  **R²:** {r2:.3f}")
...
with tab4:
    ...
    st.write("Score missing phenology data directly from the original source URLs.")
    ...
    if unscored.empty:
        ...
        st.markdown("*(Opens in a new tab so you don't lose your place)*")
        
        st.markdown("---")
        st.markdown("### Record Phenology Stages:")
