...
SEAS_MONTHLY = [
    "Tmax", "Tmin", "Tave", "PPT", "Rad", "DD_0", "DD5", 
    "DD_18", "DD18", "NFFD", "PAS", "Eref", "CMD", "RH", "CMI"
]
...
CANONICAL_COLUMNS = [
    ...
    "Photoperiod_Hours", "Phenology_Scored", "Flowering", 
    "Fruiting", "Vegetative", "URL", "Y_3Mo_prior_mean_Tave", 
    "N_3Mo_prior_mean_Tave", "Tave_Anomaly", "Y_3Mo_prior_mean_PPT", 
    "N_3Mo_prior_mean_PPT", "PPT_Anomaly"
                    ...
                    if res.status_code == 200:
                        ...
                        for obs in results:
                            ...
                            raw_records.append({
                                ...
                                "Col_Number": obs.get('recordNumber', ''), "Barcode": obs.get('catalogNumber', '') or obs.get('occurrenceID', ''), 
                                "Species": obs.get('species', gbif_spp), "Year": safe_int(y), "DOY": doy, 
                                "Latitude": obs.get('decimalLatitude'), "Longitude": obs.get('decimalLongitude'), 
                                "Elevation": obs.get('elevation', np.nan), "URL": rec_url, 
...
with st.sidebar.expander("📸 Fetch from iNaturalist", expanded=False):
    ...
    if st.button("Fetch & Process iNat", type="primary", use_container_width=True):
        with st.spinner(f"Paginating iNaturalist..."):
            ...
            raw_records, page, max_to_fetch = [], 1, max(i_limit * 10, 500)
                        ...
                        for obs in results:
                            ...
                            if obs.get('location') and obs.get('observed_on'):
                                ...
                                raw_records.append({
                                    ...
                                    "Col_Number": np.nan, "Barcode": obs.get('id', ''), "Species": obs.get('taxon', {}).get('name', inat_spp), 
...
with st.expander("⚙️ Global Analysis & Outlier Settings", expanded=True):
    if df.empty:
        ...
        focus_collector = st.checkbox(
            ...
            value=False,
            help="When checked, tabs will automatically filter to show ONLY data from your chosen collector."
        )
        ...
        # --- NEW FEATURE ENGINEERING (Circular DOY & Climate Scenario) ---
        if not plot_df.empty:
            plot_df['DOY'] = pd.to_numeric(plot_df['DOY'], errors='coerce')
            plot_df['DOY_sin'] = np.sin(2 * np.pi * plot_df['DOY'] / 365)
            plot_df['DOY_cos'] = np.cos(2 * np.pi * plot_df['DOY'] / 365)

            def categorize_climate(row):
                temp = "Warm" if pd.to_numeric(row.get('Tave_Anomaly', 0), errors='coerce') > 0 else "Cool"
                precip = "Wet" if pd.to_numeric(row.get('PPT_Anomaly', 0), errors='coerce') > 0 else "Dry"
                return f"{temp} & {precip}"

            if 'Tave_Anomaly' in plot_df.columns and 'PPT_Anomaly' in plot_df.columns:
                plot_df['Climate_Scenario'] = plot_df.apply(categorize_climate, axis=1)
...
# ==========================================
#        MAIN UI: TABS (UPDATED)
# ==========================================
tab1, tab2, tab3, tab4, tab5, tab6, tab_phenocline, tab_anomalies, tab_ml_prob = st.tabs([
    ...
    "🌎 Pheno-cline Dashboard", 
    "🌡️ Climate Anomalies", 
    "🧠 ML Probability Curve"
])
...
with tab2:
    if plot_df.empty: st.warning("No data to map.")
        ...
        if map_df.empty: st.warning("No coordinate data available in the filtered dataset.")
            ...
            map_color_by = st.radio("Color Map Points By:", options=["Species & Data Source", "Data Source (GBIF vs iNaturalist)", "Species & Outliers"], horizontal=True)
            target_color_col = 'Species_Source' if "Species & Data Source" in map_color_by else ('Data_Source' if "Data Source" in map_color_by else 'Map_Label')
...
with tab3:
    if plot_df.empty: st.info("No data available for graphing.")
        ...
        if len(valid_plot_df) >= 2:
            ...
            fig = px.scatter(
                valid_plot_df, x=selected_x, y=selected_y, color=color_var, symbol='Data_Source', 
                color_discrete_map={'Normal': '#636EFA', 'Outlier': '#EF553B'}, hover_data=['Species', 'Collector', 'Year', 'DOY', 'Data_Source'], 
            ...
            available_srcs, available_spps = trend_df['Data_Source'].dropna().unique(), trend_df['Species'].dropna().unique()
            
            for src in available_srcs: line_options.append(f"All Species ({src} Only)")
            ...
            selected_lines = st.multiselect("Draw trendlines for:", options=line_options, default=["Overall Combined"] if "Overall Combined" in line_options else [])
            ...
            def add_trendline(df_sub, name, color, dash_style):
                if len(df_sub) > 1:
                    ...
                    if np.var(x_v) > 0 and np.var(y_v) > 0:
                        ...
                        fig.add_trace(go.Scatter(x=x_seq, y=y_seq, mode='lines', name=f'{name} (R²={r2:.2f})', line=dict(color=color, width=3, dash=dash_style)))
                        return {"Dataset Line": name, "Linear Equation": f"y = {slope:.4f}x {'+' if intercept >= 0 else '-'} {abs(intercept):.4f}", "R²": f"{r2:.4f}", "p-value": f"{p_value:.4e}" if p_value < 0.0001 else f"{p_value:.4f}", "Sample Size (n)": len(df_sub)}
                return None
            ...
            for line_choice in selected_lines:
                if line_choice == "Overall Combined":
                    ...
                    stat = add_trendline(trend_df[trend_df['Data_Source'] == line_choice.replace("All Species (", "").replace(" Only)", "")], line_choice, line_palette[color_idx % len(line_palette)], 'dash')
                elif "(Combined Sources)" in line_choice:
                    stat = add_trendline(trend_df[trend_df['Species'] == line_choice.replace(" (Combined Sources)", "")], line_choice, line_palette[color_idx % len(line_palette)], 'dot')
                else:
                    for spp in available_spps:
                        for src in available_srcs:
                            if line_choice == f"{spp} ({src})":
                                stat = add_trendline(trend_df[(trend_df['Species'] == spp) & (trend_df['Data_Source'] == src)], line_choice, line_palette[color_idx % len(line_palette)], 'dashdot')
...
with tab4:
    ...
    if unscored.empty: st.success("🎉 All URL records within this focused workspace configuration have been scored!")
    else:
...
with tab5:
    ...
    if plot_df.empty: st.warning("No data available to train a model.")
    else:
        num_cols = plot_df.select_dtypes(include=['number']).columns.tolist()
        cat_cols = [c for c in ['Species', 'Data_Source'] if c in plot_df.columns]
        
        if len(num_cols) < 2: st.warning("Not enough numeric columns for machine learning.")
        else:
            ...
            if st.checkbox("🚀 Enable & Train Random Forest Model", value=False):
                if not ml_features: st.warning("Please select at least one predictor feature.")
                else:
                    ml_df = plot_df[[ml_target] + ml_features].dropna()
                    if len(ml_df) < 10: st.error("Not enough valid data points to train a reliable model. You need at least 10 complete rows.")
                    else:
                        with st.spinner("One-Hot Encoding Categoricals & Training Model..."):
                            from sklearn.ensemble import RandomForestRegressor
                            X_raw, y = ml_df[ml_features], ml_df[ml_target]
                            ...
                            fig_rf = px.bar(imp_df, x='Importance Weight', y='Feature', orientation='h', title="Top 15 Feature Importances", color='Importance Weight', color_continuous_scale="Viridis")
                            st.plotly_chart(fig_rf, use_container_width=True)

with tab6:
    st.markdown("### 🗄️ Comprehensive Database Master Ledger")
    st.info("⚠️ This dashboard exposes the raw, unfiltered master database.")
...
# --- TAB 7: PHENO-CLINE DASHBOARD ---
with tab_phenocline:
    st.subheader("Hopkin's Bioclimatic Law in Action")
    st.write("Visualizing how geographic gradients (latitude and elevation) delay phenological events.")
    
    if not plot_df.empty and 'Flowering' in plot_df.columns:
        flowering_df = plot_df[plot_df['Flowering'] == True]
        if flowering_df.empty:
            st.warning("No records marked as 'Flowering' found in the active subset.")
        else:
            col1, col2 = st.columns(2)
            with col1:
                fig_lat = px.scatter(
                    flowering_df, x="Latitude", y="DOY", trendline="ols", 
                    title="Bloom DOY vs. Latitude", opacity=0.6, color_discrete_sequence=["#2E86C1"]
                )
                st.plotly_chart(fig_lat, use_container_width=True)
                    
            with col2:
                if 'Elevation' in flowering_df.columns and flowering_df['Elevation'].notna().sum() > 0:
                    fig_elev = px.scatter(
                        flowering_df, x="Elevation", y="DOY", trendline="ols", 
                        title="Bloom DOY vs. Elevation (m)", opacity=0.6, color_discrete_sequence=["#28B463"]
                    )
                    st.plotly_chart(fig_elev, use_container_width=True)
                else:
                    st.warning("Insufficient Elevation data for mapping.")
    else:
        st.warning("Dataset is empty or missing 'Flowering' classification.")

# --- TAB 8: CLIMATE ANOMALIES ---
with tab_anomalies:
    st.subheader("How Climate Anomalies Shift the Bloom Window")
    
    if not plot_df.empty and 'Flowering' in plot_df.columns and 'Climate_Scenario' in plot_df.columns:
        flowering_df = plot_df[plot_df['Flowering'] == True]
        ...
        if flowering_df.empty:
            ...
            fig_violin = px.violin(
                flowering_df, x="Climate_Scenario", y="DOY", color="Climate_Scenario",
                box=True, points="all", title="Phenological Shifts by Climate Anomaly",
                category_orders={"Climate_Scenario": ["Cool & Wet", "Cool & Dry", "Warm & Wet", "Warm & Dry"]}
            )
            fig_violin.update_layout(xaxis_title="Climate Anomaly Type", yaxis_title="Day of Year (DOY)")
            st.plotly_chart(fig_violin, use_container_width=True)
    else:
        st.warning("Ensure the active dataset has climate anomalies generated.")

# --- TAB 9: ML PROBABILITY CURVE ---
with tab_ml_prob:
    st.subheader("Bloom Probability Curve")
    st.markdown("Using Logistic Regression trained on **circular DOY**, photoperiod, and historical temperatures to predict flowering likelihood.")
    
    if not plot_df.empty:
        features = ['DOY_sin', 'DOY_cos', 'Photoperiod_Hours', 'Y_3Mo_prior_mean_Tave']
        
        if all(f in plot_df.columns for f in features) and 'Flowering' in plot_df.columns:
            ml_prob_df = plot_df.dropna(subset=features + ['Flowering'])
            
            if len(ml_prob_df) > 10 and len(ml_prob_df['Flowering'].unique()) > 1:
                X = ml_prob_df[features]
                y = ml_prob_df['Flowering'].astype(int)
                
                clf = LogisticRegression()
                clf.fit(X, y)
                
                mock_doy = np.arange(1, 366)
                mock_X = pd.DataFrame({
                    'DOY_sin': np.sin(2 * np.pi * mock_doy / 365),
                    'DOY_cos': np.cos(2 * np.pi * mock_doy / 365),
                    'Photoperiod_Hours': [ml_prob_df['Photoperiod_Hours'].mean()] * 365,
                    'Y_3Mo_prior_mean_Tave': [ml_prob_df['Y_3Mo_prior_mean_Tave'].mean()] * 365
                })
                
                probabilities = clf.predict_proba(mock_X)[:, 1]
                
                fig_prob = go.Figure()
                fig_prob.add_trace(go.Scatter(
                    x=mock_doy, y=probabilities, mode='lines', name='Probability of Bloom',
                    line=dict(color='firebrick', width=3)
                ))
                fig_prob.update_layout(
                    title="Flowering Probability Over the Year", 
                    xaxis_title="Day of Year", yaxis_title="Probability of Flowering (0 to 1)"
                )
                st.plotly_chart(fig_prob, use_container_width=True)
                st.success("Logistic Regression Model trained successfully!")
            else:
                st.warning("Not enough variance (need both flowering and non-flowering records) to train the probability curve.")
        else:
            st.warning("Missing required features for the Probability Curve in the active dataset.")
    else:
